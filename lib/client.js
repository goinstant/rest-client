 ...
 * @param {function} cb
 */
Client.prototype.req = function(response, opts, cb) {
  if (_.isFunction(opts)) {
    cb = opts;
  }

  var tasks = [
    this._performRequest.bind(this, response, opts)
  ];
  ...
  async.series(tasks, function(err, results) {
    ...
    var body = requestResult && requestResult[0] || null;
    var response = requestResult && requestResult[1] || null;
 ...
 * @param {function} cb
 */
Client.prototype._performRequest = function(response, opts, cb) {
  var pipe = true;
...
Client.prototype._inspectBody = function(res, body, cb) {
  ...
  var hasRes = !!res;
  var deposition = res.headers && res.headers['content-disposition'];
