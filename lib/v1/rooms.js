...
function validateRoomCreate(opts, cb) {
  if (!_.isObject(opts)) {
    throw new Error('Parameter opts must be an object');
  }
  if (!_.isString(opts.room_name)) {
    throw new Error('Opts property room_name must be a string');
  }
  if (!(_.isNumber(opts.app_id) || _.isString(opts.app_id))) {
    throw new Error('Opts property app_id must be an integer or string');
  }
  if (!_.isFunction(cb)) {
    throw new Error('Callback must be supplied');
  }
